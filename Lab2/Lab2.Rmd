...
```{r warning=FALSE}
elc_habitat<-st_read(here::here("Data","elc_habitat.shp")) # i like this! Nice way of reading in a spatial file
humanaccess<-st_read(here::here("Data","humanacess.shp"))

plot(elc_habitat) 
<<<<<<< HEAD:Lab2/README.Rmd
tmap_mode("plot") # plot = permanent map you can't zoom into, view = user-friendly map 
=======

tmap_mode("plot") 
>>>>>>> 72ba2fd1398b463cb5fe5f14ab75ebc5716c6fa6:Lab2/Lab2.Rmd
tm_shape(elc_habitat) + tm_sf()
tm_shape(humanaccess) + tm_sf()

wolfyht<-st_read(here::here("Data","wolfyht.shp")) # this is a simple feature geometry 
head(wolfyht)
str(wolfyht)

tm_shape(wolfyht) + tm_sf()
...
<<<<<<< HEAD:Lab2/README.Rmd
tm_shape(elc_habitat) + tm_sf("MOOSE_W", border.alpha = 0) # ahh so cool!
...
#construct ggplot2 plot for Moose Winter Habitat
sheep_plot<-ggplot() + 
  geom_sf(data = elc_habitat, mapping = aes(fill = as.factor(SHEEP_W)), color = NA) + labs(x="Easting",y="Northing") + theme(axis.text.y = element_text(angle = 90, hjust=0.5)) + theme_
...
note that adehabitat requires sp package 
```{r warning=FALSE}
...
class(rd)
# Fit 99% mpc
#cp.rd <- mcp(rd, percent=99) <- MCP function needs at least 5 locations to do a home range 
...
names(rd)<-"NAME"
# @ function tells where to go in sp.points dataframe 
...
# essentially modifies how many points we use to calculate an mcp, and shows us at what point it's asymptoted 
...
homerangeRD <- getverticeshr(red.deerUD) # gets polygons from KDE 
as.data.frame(homerangeRD)
class(homerangeRD)
plot(homerangeRD, col=2:4) # 95th percentile of kernel home ranges 
...
contour(xyzv, add=TRUE) # adds on contour lines for various percentiles 
```
...
contour(xyzv, add=TRUE) # wow, where are the red deer wolves? They don't show up? 

#let's check out how many telemetry locations exist for red deer/bow valley wolves....
table(wolfyht$Pack) 
# oh crikey! This explains it 
```
...
wolfused <- na.omit(wolfused) # gets rid of any rows with a single NA 
summary(wolfused)
dim(wolfused)
```
...
par(mfrow = c(1,1))
# Plot Bow Valley
hist(wolfused$Elevation2[wolfused$pack=="Bow Valley"],breaks=50, xlim = c(1400,2250), probability = TRUE, main="Wolf Habitat Selection", xlab="Elevation", col="white") 
